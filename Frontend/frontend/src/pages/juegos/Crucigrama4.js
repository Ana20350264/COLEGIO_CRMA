...
function HomePage() {
  ...
  const crosswordStructure = [
    { word: "USUALLY", direction: "vertical", startIndex: 6 },
    { word: "DOES", direction: "vertical", startIndex: 8 },
    { word: "DO", direction: "horizontal", startIndex: 8 },
    { word: "NEVER", direction: "vertical", startIndex: 18 },
    { word: "EVERYDAY", direction: "horizontal", startIndex: 24 },
  ];
...
const userWords = {
  15: [answers[6], answers[14], answers[22], answers[30], answers[38], answers[46], answers[54]].map((answer, index) => {
    console.log(`Palabra 15: Celda ${index} -> ${answer}`); // Imprime cada celda para la palabra "WAKE"
    return answer;
  }).join(""), // "WAKE"
  
  16: [answers[8], answers[16], answers[24], answers[32]].map((answer, index) => {
    console.log(`Palabra 16: Celda ${index} -> ${answer}`); // Imprime cada celda para la palabra "WALK"
    return answer;
  }).join(""), // WALK
  
  17: [answers[8], answers[9]].map((answer, index) => {
    console.log(`Palabra 17: Celda ${index} -> ${answer}`); // Imprime cada celda para la palabra "PAINT"
    return answer;
  }).join(""), // EAT

  18: [answers[18], answers[26], answers[34], answers[42], answers[50]].map((answer, index) => {
    console.log(`Palabra 18: Celda ${index} -> ${answer}`); // Imprime cada celda para la palabra "CINEMA"
    return answer;
  }).join(""), // GO
  
  19: [answers[24], answers[25], answers[26], answers[27], answers[28], answers[29], answers[30], answers[31]].map((answer, index) => {
    console.log(`Palabra 19: Celda ${index} -> ${answer}`); // Imprime cada celda para la palabra "MUSIC"
    return answer;
  }).join(""), // DRINK
};
    ...
    <HomeContainer>
      ...
      >
        ...
        <PistasContainer>
          ...
          <h3>Vertical</h3>
          <Pista>
            <strong>1. Generalmente:</strong> Palabra que describe algo que ocurre frecuentemente.
          </Pista>
          <Pista>
            <strong>2. Hace:</strong> Verbo auxiliar usado en preguntas y respuestas con "he", "she" y "it".
          </Pista>
          <Pista>
            <strong>4. Nunca:</strong> Frecuencia que indica que algo no sucede en absoluto.
          </Pista>
          <br />
          <h3>Horizontal</h3>
          <Pista>
            <strong>2. Hacer:</strong> Verbo auxiliar usado en preguntas y respuestas con "I", "you", "we" y "they".
          </Pista>
          <Pista>
            <strong>5. Todos los dias:</strong> Frecuencia con la que algo sucede como parte de una rutina.
          </Pista>
            </PistasContainer>
      ...
      <NavigationButtons>
        <button>
          <Link to="/unir4">Anterior</Link>
        </button>
        <button>
          <Link to="/subtema5">Siguiente</Link>
        </button>
      </NavigationButtons>
      <Footer />
    </HomeContainer>

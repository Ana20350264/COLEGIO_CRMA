...
function HomePage() {
  ...
  const crosswordStructure = [
    { word: "NEWS", direction: "horizontal", startIndex: 1 },
    { word: "SUSCRIBE", direction: "vertical", startIndex: 4 },
    { word: "STREAM", direction: "vertical", startIndex: 6 },
    { word: "ARTICLE", direction: "horizontal", startIndex: 24 },
    { word: "RADIO", direction: "vertical", startIndex: 25 },
  ];
...
const userWords = {
  20: [answers[1], answers[2], answers[3], answers[4]].map((answer, index) => {
    console.log(`Palabra 20: Celda ${index} -> ${answer}`); // Imprime cada celda para la palabra "WAKE"
    return answer;
  }).join(""), // "WAKE"
  
  21: [answers[4], answers[12], answers[20], answers[28], answers[36], answers[44], answers[52], answers[60]].map((answer, index) => {
    console.log(`Palabra 21: Celda ${index} -> ${answer}`); // Imprime cada celda para la palabra "WALK"
    return answer;
  }).join(""), // WALK
  
  22: [answers[6], answers[14], answers[22], answers[30], answers[38], answers[46]].map((answer, index) => {
    console.log(`Palabra 22: Celda ${index} -> ${answer}`); // Imprime cada celda para la palabra "PAINT"
    return answer;
  }).join(""), // EAT

  23: [answers[24], answers[25], answers[26], answers[27], answers[28], answers[29], answers[30]].map((answer, index) => {
    console.log(`Palabra 23: Celda ${index} -> ${answer}`); // Imprime cada celda para la palabra "CINEMA"
    return answer;
  }).join(""), // GO
  
  24: [answers[25], answers[33], answers[41], answers[49], answers[57]].map((answer, index) => {
    console.log(`Palabra 24: Celda ${index} -> ${answer}`); // Imprime cada celda para la palabra "MUSIC"
    return answer;
  }).join(""), // DRINK
};
    ...
    <HomeContainer>
      ...
      >
        ...
        <PistasContainer>
          ...
          <h3>Vertical</h3>
          <Pista>
            <strong>2. Suscribirse:</strong> Acción de seguir regularmente a un medio o canal.
          </Pista>
          <Pista>
            <strong>3. Transmitir:</strong> Ver o escuchar contenido en línea en tiempo real.
          </Pista>
          <Pista>
            <strong>5. Radio:</strong> Medio de comunicación que transmite audio.
          </Pista>
          <br />
          <h3>Horizontal</h3>
          <Pista>
            <strong>1. Noticias:</strong> Información sobre eventos recientes.
          </Pista>
          <Pista>
            <strong>4. Articulo:</strong> Texto escrito en un periódico o revista.
          </Pista>
        </PistasContainer>
      ...
      <NavigationButtons>
        <button>
          <Link to="/unir5">Anterior</Link>
        </button>
        <button>
          <Link to="/subtema6">Siguiente</Link>
        </button>
      </NavigationButtons>
      <Footer />
    </HomeContainer>
